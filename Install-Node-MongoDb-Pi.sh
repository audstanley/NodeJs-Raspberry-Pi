...
#Next, Creates directory for downloads, and downloads node 5.6.0
cd ~/ && mkdir temp && cd temp && wget https://nodejs.org/dist/latest-v5.x/node-v5.6.0-linux-armv7l.tar.gz;
tar -xzf node-v5.6.0-linux-armv7l.tar.gz;
#Remove the tar after extracing it.
sudo rm node-v5.6.0-linux-armv7l.tar.gz;
#This next line will copy Node over to the appropriate folder.
sudo mv node-v5.6.0-linux-armv7l/ /opt/nodejs/;
#This line will remove the nodeJs we downloaded.
sudo rm -R node-v5.6.0-linux-armv7l.tar.gz/* && sudo rmdir node-v5.6.0-linux-armv7l.tar.gz/;
