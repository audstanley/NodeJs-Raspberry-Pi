...
#Next, Creates directory for downloads, and downloads node 5.11.1
cd ~/ && mkdir temp && cd temp && wget https://nodejs.org/dist/latest-v5.x/node-v5.11.1-linux-armv7l.tar.gz;
tar -xzf node-v5.11.1-linux-armv7l.tar.gz;
#Remove the tar after extracing it.
sudo rm node-v5.11.1-linux-armv7l.tar.gz;
#This next line will copy Node over to the appropriate folder.
sudo mv node-v5.11.1-linux-armv7l/ /opt/nodejs/;
...
sudo ln -sf /opt/nodejs/bin/node /usr/sbin/node;
sudo ln -sf /opt/nodejs/bin/node /sbin/node;
sudo ln -sf /opt/nodejs/bin/node /usr/local/bin/node;
sudo ln -sf /opt/nodejs/bin/npm /usr/bin/npm;
sudo ln -sf /opt/nodejs/bin/npm /usr/sbin/npm;
sudo ln -sf /opt/nodejs/bin/npm /sbin/npm;
